...
# ─── Save JSON ────────────────────────────────────────────────────────────────
def save_json(data: JSONSerializable, filepath: str | Path, folder: str | Path | None = None, indent: int = 4) -> bool:
...
# ─── Load JSON ────────────────────────────────────────────────────────────────
def load_json(filepath: str | Path, folder: str | Path | None = None) -> JSONSerializable | None:
...
def save_pydantic_model(
    model: BaseModel, filepath: str | Path, folder: str | Path | None = None, indent: int = 2
) -> bool:
    ...
    try:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text(model.model_dump_json(indent=indent, exclude_none=True), encoding="utf-8")
...
def load_pydantic_model[T: BaseModel](
    model_class: type[T], filepath: str | Path, folder: str | Path | None = None
) -> T | None:
