...
def get_project_root() -> Path:
    ...
    This function searches for common project indicator files (e.g.,
    pyproject.toml, .git) starting from the current working directory and
...
class ConfigPaths(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="CONFIG_", case_sensitive=False, extra="ignore", env_file=get_default_env_path()
    )
...
class TagSettings(BaseSettings):
    ...
    id_attr_str: TYPE_UUID | None = Field(
        default=None, title="Strength Attr Tag ID", description="Tag UUID for Strength attribute"
    )
    id_attr_int: TYPE_UUID | None = Field(
        default=None, title="Intelligence Attr Tag ID", description="Tag UUID for Intelligence attribute"
    )
    id_attr_con: TYPE_UUID | None = Field(
        default=None, title="Constitution Attr Tag ID", description="Tag UUID for Constitution attribute"
    )
    id_attr_per: TYPE_UUID | None = Field(
        default=None, title="Perception Attr Tag ID", description="Tag UUID for Perception attribute"
    )
    id_no_attr: TYPE_UUID | None = Field(
        default=None, title="No Attr Tag ID", description="Tag UUID for attribute not manually set"
    )
    id_legacy: TYPE_UUID | None = Field(
        default=None, title="Legacy Tag ID", description="Tag UUID for guild challenges"
    )
    id_challenge: TYPE_UUID | None = Field(
        default=None, title="Challenge Tag ID", description="Tag UUID for challenge tasks"
    )
    id_personal: TYPE_UUID | None = Field(
        default=None, title="Personal Tag ID", description="Tag UUID for personal tasks"
    )
    ...
    def validate_tag_id_uuid(cls, v: str | TYPE_UUID | None) -> TYPE_UUID | None:
        ...
        if isinstance(v, str):
            ...
            try:
                ...
                raise ValueError("Tag ID must be a valid UUID format") from e

        raise TypeError(f"Unsupported type for UUID field: {type(v).__name__}")
    ...
    def validate_default_attr(cls, v: str | None) -> str | None:
        ...
        if v_lower not in allowed_values:
            raise ValueError(f"default_attr must be one of: {', '.join(allowed_values)}")
...
class ApplicationSettings(BaseSettings):
    ...
    def get_schema_friendly(cls) -> dict[str, Any]:
        ...
        This schema is designed for user-friendly display, grouping settings
        by their component models and providing more human-readable details.
    ...
    def _build_friendly_schema(cls, schema: dict[str, Any]) -> dict[str, Any]:
        """Builds a simplified and normalized schema for display purposes.
    ...
    def _extract_field_info(field_name: str, schema_field_data: dict[str, Any], section_name: str) -> dict[str, Any]:
        """Extracts detailed field information similar to FieldInfo.from_schema_field().
    ...
    def _extract_ref(item_data: dict[str, Any]) -> str | None:
        """Extracts the reference name from a schema item.
    ...
    def _normalize_properties(props: dict[str, Any]) -> dict[str, Any]:
        """Normalizes properties by resolving 'anyOf' for friendly schema.

        This helps in cases where a field can be of multiple types (e.g., type or null).
