...
class TagMixin:
    ...
    def _validate_tag_name(self, tag_name: str) -> str:
        """Validates that the tag name is not empty and returns stripped version.
    ...
    def _operation_successful_check(self, api_result: Any) -> bool:
        """Determines if a tag action (often returning 204 No Content) was successful.
    ...
    async def get_all_tags_raw_response(self) -> HabiticaResponse:
        """Fetches all user tags, returning the full HabiticaResponse object.
    ...
    async def get_all_tags_data(self) -> list[dict[str, Any]]:
        """Fetches all user tags, returning only the 'data' field from the API response.
