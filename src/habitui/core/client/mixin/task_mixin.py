...
class TaskMixin:
    ...
    def _validate_not_empty_param(self, value: str, param_name: str) -> str:
        """Validate a string parameter is not empty and returns its stripped version.
    ...
    def _normalize_attribute_parameter(self, attribute: Attribute | str) -> str:
        ...
        if isinstance(attribute, str) and attribute.lower() in {
            "str",
            "int",
            "con",
            "per",
    ...
    def _operation_successful_check(self, api_result: Any) -> bool:
        """Determine if a task action (often returning 204 No Content) was successful.
    ...
    async def get_user_tasks_raw_response(
        ...
        *,
        ...
        | Literal["habits", "dailys", "todos", "rewards"]
        | None = None,
    ) -> HabiticaResponse:
        ...
        if task_type_filter:
            type_value_str = (
                task_type_filter.value
                if isinstance(task_type_filter, TaskType)
                else task_type_filter
            )
            type_mapping = {
                "habit": "habits",
                "daily": "dailys",
            ...
            api_type_param = type_mapping.get(
                type_value_str.lower(),
                type_value_str.lower(),
            )
    ...
    async def score_task_action(
        self,
        task_id: str,
        score_direction: ScoreDirection | Literal["up", "down"] = ScoreDirection.UP,
    ) -> dict[str, Any]:
    ...
    async def assign_task_attribute(
        self,
        task_id: str,
        task_attribute: Attribute | Literal["str", "int", "con", "per"],
    ) -> dict[str, Any]:
        ...
        result = await self.update_existing_task(
            task_id,
            {"attribute": attribute_value_str},
        )
        return cast("dict[str, Any]", result)

    async def move_task_to_new_position(
        self,
        task_id: str,
    ...
    async def add_tag_to_task(self, task_id: str, tag_id_to_add: str) -> dict[str, Any]:
        ...
        log.info(
            f"Executing add_tag_to_task: taskId={task_id}, tagId={validated_tag_id}",
        )
    ...
    async def add_checklist_item_to_task(
        self,
        task_id: str,
        item_text: str,
    ) -> dict[str, Any]:
        ...
        """
        self._validate_not_empty_param(task_id, "Task ID")
        validated_text = self._validate_not_empty_param(
            item_text,
            "Checklist item text",
        )
    ...
    async def update_checklist_item_on_task(
        ...
        """
        self._validate_not_empty_param(task_id, "Task ID")
        validated_item_id = self._validate_not_empty_param(
            checklist_item_id,
            "Checklist Item ID",
        )
        validated_new_text = self._validate_not_empty_param(
            new_text,
            "New checklist item text",
        )
        result = await self.put(
            f"tasks/{task_id}/checklist/{validated_item_id}",
            data={"text": validated_new_text},
        )
        return cast("dict[str, Any]", result)

    async def delete_checklist_item_from_task(
        self,
        task_id: str,
        checklist_item_id: str,
    ) -> dict[str, Any]:
    ...
    async def score_checklist_item_on_task(
        self,
        task_id: str,
        checklist_item_id: str,
    ) -> dict[str, Any]:
