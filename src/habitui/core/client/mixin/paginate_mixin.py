...
class BasePaginationUtilitiesMixin:
    ...
    async def _fetch_all_pages_incrementally(  # noqa: C901
        ...
        while True:
            ...
            log.debug(
                "Pagination: Page {} result type: {}",
                current_page_number,
                type(page_result),
            )
            if isinstance(page_result, list):
                log.debug(
                    "Pagination: Page {} list length: {}",
                    current_page_number,
                    len(page_result),
                )
            ...
            if page_result is None:
                ...
                isinstance(page_result, ChallengeCollection)
                and not page_result.challenges
            ):
                is_empty = True
            ...
            if is_empty:
                log.debug(
                    "Pagination: Page {} considered empty. Stopping.",
                    current_page_number,
                )
                break
    ...
    async def _collect_all_items_from_paginated_source(
        ...
        async for page_data_object in self._fetch_all_pages_incrementally(
            page_fetcher_callable,
        ):
            if page_data_object is not None:
                try:
                    ...
                    if isinstance(items_from_this_page, list):
                        ...
                        log.warning(
                            "Pagination: items_extractor_from_page did not return a list.",
                        )
                except Exception as e:
                    log.error(
                        "Pagination: Error extracting items from page data: {}",
                        e,
                        exc_info=True,
                    )
    ...
    async def get_all_user_challenges_data(
        ...
        *,
        ...
        return await self._collect_all_items_from_paginated_source(
            ...
            items_extractor_from_page=lambda page_data: (
    ...
    async def iterate_all_user_challenges_data(
        ...
        *,
        ...
        async for page_data_list in self._fetch_all_pages_incrementally(
            lambda p: self.get_user_challenges_data(
                member_only=member_only,
                owned_filter=owned_filter,
                page=p,
            ),
...
class InboxPaginationMixin(BasePaginationUtilitiesMixin):
    ...
    async def get_all_inbox_messages_raw_responses(
        ...
        *,
        ...
        responses: list[HabiticaResponse] = [
            page_response
            async for page_response in self._fetch_all_pages_incrementally(
                lambda p: self.get_inbox_messages_raw_response(
    ...
    async def get_all_inbox_messages_data(
        self,
        *,
        conversation_id: str | None = None,
    ) -> list[dict[str, Any]]:
    ...
    async def iterate_all_inbox_messages_data(
        ...
        *,
        ...
        """
        async for page_data_list in self._fetch_all_pages_incrementally(
            lambda p: self.get_inbox_messages_data(
                conversation_id=conversation_id,
                page_number=p,
            ),
        ):
