...
class BasePaginationUtilitiesMixin:
    ...
    async def _fetch_all_pages_incrementally(  # noqa: C901
        self, page_fetcher_callable: Callable[[int], Awaitable[Any]]
    ) -> AsyncIterator[Any]:
        """Fetches pages and yields each page's result until an empty or invalid page is encountered.
    ...
    async def _collect_all_items_from_paginated_source(
        ...
        """Collects all individual items from a paginated source into a single list.
...
class ChallengePaginationMixin(BasePaginationUtilitiesMixin):
    """Provides methods to fetch all pages of Habitica challenges."""
    ...
    async def get_all_user_challenges_raw_responses(
        self, *, member_only: bool = True, owned_filter: str | None = None
    ) -> list[HabiticaResponse]:
        """Fetches all pages of challenges and returns them as a list of raw HabiticaResponse objects.
        ...
        responses: list[HabiticaResponse] = [
            ...
            async for page_response in self._fetch_all_pages_incrementally(
                lambda p: self.get_user_challenges_raw_response(
                    member_only=member_only, page=p, owned_filter=owned_filter
                )
            )
    ...
    async def iterate_all_user_challenges_raw_responses(
        ...
        """Iterates through all pages of challenges, yielding raw HabiticaResponse objects.
    ...
    async def get_all_user_challenges_data(
        ...
        """Fetches all challenges and returns them as a list of dictionaries (data field).
        ...
        return await self._collect_all_items_from_paginated_source(
            page_fetcher_callable=lambda p: self.get_user_challenges_data(
                member_only=member_only, owned_filter=owned_filter, page=p
            ),
    ...
    async def iterate_all_user_challenges_data(
        ...
        """Iterates through all challenges, yielding individual challenge data dictionaries.
...
class InboxPaginationMixin(BasePaginationUtilitiesMixin):
    """Provides methods to fetch all pages of Habitica inbox messages."""
    ...
    async def get_all_inbox_messages_raw_responses(
        ...
        """Fetches all pages of inbox messages and returns them as a list of raw HabiticaResponse objects.
    ...
    async def iterate_all_inbox_messages_raw_responses(
        ...
        """Iterates through all pages of inbox messages, yielding raw HabiticaResponse objects.
    ...
    async def get_all_inbox_messages_data(self, *, conversation_id: str | None = None) -> list[dict[str, Any]]:
        """Fetches all inbox messages and returns them as a list of dictionaries (data field).
        ...
        return await self._collect_all_items_from_paginated_source(
            page_fetcher_callable=lambda p: self.get_inbox_messages_data(
                conversation_id=conversation_id, page_number=p
            ),
            items_extractor_from_page=lambda page_data: (page_data if isinstance(page_data, list) else []),
        )
    ...
    async def iterate_all_inbox_messages_data(
        ...
        """Iterates through all inbox messages, yielding individual message data dictionaries.
