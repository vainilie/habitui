...
class PartyMixin:
    """Provides methods for interacting with the Habitica API's party endpoints."""
    ...
    def _operation_successful_check(self, api_result: Any) -> bool:
        """Determines if a party action (often POST) was successful.
    ...
    async def get_current_party_raw_response(self) -> HabiticaResponse:
        """Gets the current user's party data, returning the full HabiticaResponse object.

        :return: The full HabiticaResponse object for the current party.
        """
        return cast("HabiticaResponse", await self.get("/groups/party", return_full_response_object=True))

    async def get_current_party_data(self) -> dict[str, Any]:
        """Gets the current user's party data, returning only the 'data' field from the API response.
    ...
    async def get_group_chat_messages_raw_response(self, group_id: str = "party") -> HabiticaResponse:
        """Fetches chat messages for a specific group, returning the full HabiticaResponse. Defaults to 'party'.
        ...
        return cast("HabiticaResponse", await self.get(f"/groups/{group_id}/chat", return_full_response_object=True))

    async def get_group_chat_messages_data(self, group_id: str = "party") -> list[dict[str, Any]]:
        """Fetches chat messages for a group, returning the 'data' field (list of messages).
    ...
    async def delete_group_chat_message(self, group_id: str, chat_id: str) -> bool:
        """Deletes a specific chat message within a group.
    ...
    async def mark_group_chat_as_seen(self, group_id: str = "party") -> bool:
        """Marks messages in a group chat as seen by the current user.
    ...
    async def post_message_to_group_chat(
        self, group_id: str = "party", message_content: str = ""
    ) -> SuccessfulResponseData:
        """Posts a message to a specified group chat.
    ...
    async def accept_party_quest_invite(self, group_id: str = "party") -> dict[str, Any]:
        """Accepts a pending quest invitation for the specified group (usually current party).
    ...
    async def reject_party_quest_invite(self, group_id: str = "party") -> dict[str, Any]:
        """Rejects a pending quest invitation for the specified group.
    ...
    async def leave_active_party_quest(self, group_id: str = "party", keep_tasks: bool = True) -> dict[str, Any]:
        """Leaves an active quest without leaving the party itself.
    ...
    async def abort_active_party_quest(self, group_id: str = "party") -> dict[str, Any]:
        """Aborts an active quest (usually only possible by the quest leader/party leader).
    ...
    async def cast_skill_on_target(self, spell_key: str, target_id: str | None = None) -> SuccessfulResponseData:
