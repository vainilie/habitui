...
class ChallengeMixin:
	...
	async def get_user_challenges_raw(
		...
		*,
		...
		return cast(
			"HabiticaResponse",
			await self.get("/challenges/user", params=params, return_full_response_object=True),
		)
	...
	async def get_challenge_tasks_raw(self, challenge_id: str) -> HabiticaResponse:
		...
		return cast(
			"HabiticaResponse",
			await self.get(f"/tasks/challenge/{challenge_id}", return_full_response_object=True),
		)
	...
	async def unlink_task_from_challenge(
		self,
		task_id: str,
		task_handling_option: TaskKeepOption | Literal["keep", "remove"] = TaskKeepOption.KEEP,
	) -> bool:
