...
class ChallengeMixin:
    ...
    def _normalize_challenge_task_keep_option(
        self,
        keep_option_input: ChallengeTaskKeepOption | str,
    ) -> str:
        ...
        if isinstance(keep_option_input, str) and keep_option_input in {
            "keep-all",
            "remove-all",
        }:
    ...
    def _normalize_task_keep_option(
        self,
        keep_option_input: TaskKeepOption | str,
    ) -> str:
    ...
    def _validate_challenge_creation_dict(
        self,
        ...
        missing = [
            field for field in required_fields if not challenge_payload.get(field)
        ]
    ...
    def _operation_successful_check(self, api_result: Any) -> bool:
        ...
        return api_result is None or (
            isinstance(api_result, dict | list) and not api_result
        )
    ...
    async def get_challenge_tasks_raw(self, challenge_id: str) -> HabiticaResponse:
        ...
        return cast(
            "HabiticaResponse",
            await self.get(
                f"/tasks/challenge/{challenge_id}",
                return_full_response_object=True,
            ),
        )
    ...
    async def leave_challenge(
        ...
        keep_value_str = self._normalize_challenge_task_keep_option(
            task_handling_option,
        )
        result = await self.post(
            f"/challenges/{challenge_id}/leave",
            params={"keep": keep_value_str},
        )
    ...
    async def unlink_task_from_challenge(
        self,
        task_id: str,
        task_handling_option: TaskKeepOption
        | Literal["keep", "remove"] = TaskKeepOption.KEEP,
    ) -> bool:
    ...
    async def unlink_all_tasks_from_challenge(
        ...
        result = await self.post(
            f"/tasks/unlink-all/{challenge_id}",
            params={"keep": keep_value_str},
        )
        return self._operation_successful_check(result)
    ...
    async def create_new_challenge(
        self,
        challenge_payload: ChallengeCreate | dict[str, Any],
    ) -> dict[str, Any]:
    ...
    async def create_task_in_challenge(
        self,
        challenge_id: str,
        task_payload: TaskCreatePayload | dict[str, Any],
