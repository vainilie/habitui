...
class RateLimiter:
    ...
    def __init__(self, initial_interval: float = MIN_REQUEST_INTERVAL_SECONDS) -> None:
        ...
        log.debug("RateLimiter initialized with interval: {:.2f}s", self._current_interval)

    async def wait_if_needed(self) -> None:
        """Pauses execution if the time since the last request is less than the current interval.
    ...
    def update_rules_from_headers(self, response_headers: httpx.Headers) -> None:
        """Updates the rate limiting interval based on information from API response headers.
        ...
        if retry_after_seconds:
            try:
                ...
                self._current_interval = max(MIN_REQUEST_INTERVAL_SECONDS / 2, new_interval)
                log.warning(
        ...
        if remaining_requests_str and limit_per_window_str:
            try:
                ...
                if limit > 0 and (remaining / limit) < lowest_threshold:
                    ...
                    self._current_interval = max(self._current_interval, new_suggested_interval)
                    log.warning(
...
class RequestExecutionStats:
    """Tracks statistics about API request executions."""

    def __init__(self) -> None:
        """Initializes the RequestExecutionStats tracker."""
    ...
    def record_successful_request(self, duration_seconds: float) -> None:
        """Records a successfully completed API request.
    ...
    def average_response_time_seconds(self) -> float:
        """Calculates the average response time for successful requests.
    ...
    def get_summary_dict(self) -> dict[str, Any]:
        """Returns the current request statistics as a dictionary.
