...
import json
import time
from typing import TYPE_CHECKING, Any, Self, Literal, TypeVar, overload
import asyncio
...
class HabiticaAPI:
    ...
    def __init__(self, config_override: Any | None = None) -> None:
        ...
        if not (
            ...
            log.critical(
                "HabiticaAPI: API configuration is missing or invalid (user_id, api_token).",
            )
        ...
        if not (self.user_id and self.api_token):
            log.critical(
                "HabiticaAPI: User ID or API Token is missing after configuration load.",
            )
        ...
        log.debug(
            "HabiticaAPI client initialized for user {}... Base URL: {}",
            str(self.user_id)[:8],
            self.base_api_url,
        )
    ...
    def get_current_rate_limit_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "time_since_last_request_s": round(
                time.monotonic() - self._rate_limiter._last_request_time,  # noqa: SLF001
                3,
            ),
            "request_stats": self._request_stats.get_summary_dict(),
        }

    async def _execute_request(  # noqa: C901, PLR0912
        self,
        http_method: str,
        api_endpoint: str,
        ...
        try:
            ...
            if response.status_code == CODE_RATE_LIMIT_EXCEDED:
                retry_after_str = response.headers.get(
                    "Retry-After",
                    str(self._rate_limiter._current_interval * 1.5),  # noqa: SLF001
                )
                try:
            ...
            try:
                full_habitica_response = HabiticaResponse.model_validate(
                    response_json_data,
                )
                ...
                raise HabiticaAPIError(
                    message=(
                        f"Could not validate the base API response structure: {pydantic_err}"
                    ),
                    status_code=response.status_code,
            ...
            if not full_habitica_response.success:
                ...
                log.warning(
                    ...
                    error_message,
                    normalized_endpoint,
                    full_habitica_response.errors,
                )
                raise HabiticaAPIError(
                    message=(
                        full_habitica_response.message
                        or "API request failed but no message provided."
                    ),
                    status_code=response.status_code,
            ...
            try:
                ...
                if isinstance(error_response_data, dict):
                    ...
                    error_message_detail = (
                        hab_err_resp.message
                        or hab_err_resp.error
            ...
            raise HabiticaAPIError(
                message=(
                    f"API request transport error: {transport_err.__class__.__name__} - {transport_err}"
                ),
            ) from transport_err
        except Exception as e:
            self._request_stats.record_failed_request()
            log.exception(
                "Unexpected error during API request to {}: {}",
                normalized_endpoint,
                e,
            )
    ...
    def _prepare_request_data(self, data: Any | None) -> dict[str, Any] | None:
        ...
        if isinstance(data, dict):
            return data
        log.warning(
            "Request data is not a Pydantic model or dict, attempting to pass as is: {}",
            type(data).__name__,
        )
        return data
