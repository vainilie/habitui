...
class ChallengeVault(BaseVault[ChallengeCollection]):
    ...
    This vault handles the storage and retrieval of ChallengeInfo models and
    their associated tasks, providing specialized methods for challenge
    management including task categorization and user/task challenge relationships.
    """
    ...
    def get_model_configs(self) -> dict[str, type[HabiTuiSQLModel]]:
        ...
        :returns: A dictionary mapping content type strings to their respective
            HabiTuiSQLModel classes.
        """
        return {
            "challenge": ChallengeInfo,
            "user_challenge": ChallengeInUser,
            "task_challenge": ChallengeInTask,
            "challenge_daily": ChallengeTaskDaily,
            "challenge_habit": ChallengeTaskHabit,
            "challenge_reward": ChallengeTaskReward,
            "challenge_todo": ChallengeTaskTodo,
        }

    def save(self, content: ChallengeCollection, strategy: SaveStrategy = "smart", debug: bool = False) -> None:
        """Saves challenge content to the database using a specified strategy.

        This method performs a full sync of all challenge-related data.
        ...
        with Session(self.engine) as session:
            ...
            if content.challenges:
                self._save_item_list(
                    session, ChallengeInfo, content.challenges, strategy, "challenge", debug=debug, append_mode=False
                )
            if content.user_challenges:
                self._save_item_list(
                    session,
                    ChallengeInUser,
                    content.user_challenges,
                    strategy,
                    "user_challenge",
                    debug=debug,
                    append_mode=False,
            ...
            if content.challenge_tasks_reward:
                self._save_item_list(
                    session,
                    ChallengeTaskReward,
                    content.challenge_tasks_reward,
                    strategy,
                    "challenge_reward",
                    debug=debug,
                    append_mode=False,
    ...
    def save_challenge_tasks_only(
        self, tasks: dict[str, list], strategy: SaveStrategy = "smart", debug: bool = False
    ) -> None:
        """Saves only challenge tasks without affecting main challenges.

        This is ideal for updating task data independently.
    ...
    def save_user_challenges_only(
        self, user_challenges: list[ChallengeInUser], strategy: SaveStrategy = "smart", debug: bool = False
    ) -> None:
        """Saves only user challenges without affecting other data.
        ...
        with Session(self.engine) as session:
            self._save_item_list(
                session, ChallengeInUser, user_challenges, strategy, "user_challenge", debug=debug, append_mode=True
            )
            session.commit()

    def load(self) -> ChallengeCollection:
        """Loads all challenge content from the database.
        ...
        with Session(self.engine) as session:
            ...
            return ChallengeCollection(
                ...
                challenge_tasks_daily=challenge_tasks_daily,
                challenge_tasks_habit=challenge_tasks_habit,
                challenge_tasks_reward=challenge_tasks_reward,
                challenge_tasks_todo=challenge_tasks_todo,
            )

    def load_challenges_by_user_id(self, user_id: str) -> list[ChallengeInfo]:
        """Loads challenges where the user is the owner or participant.

        :param user_id: The ID of the user.
        :returns: A list of ChallengeInfo objects related to the user.
        """
        with Session(self.engine) as session:
            owned_challenges = list(
                session.exec(select(ChallengeInfo).where(col(ChallengeInfo.leader_id) == user_id)).all()
            )
            user_challenge_ids = [uc.id for uc in session.exec(select(ChallengeInUser)).all()]
            participated_challenges = []
            if user_challenge_ids:
                participated_challenges = list(
                    session.exec(select(ChallengeInfo).where(col(ChallengeInfo.id).in_(user_challenge_ids))).all()
                )
    ...
    def get_tasks_by_challenge_id(self, challenge_id: str) -> dict[str, list]:
        ...
        with Session(self.engine) as session:
            dailys = list(
                session.exec(
                    select(ChallengeTaskDaily).where(col(ChallengeTaskDaily.challenge_id) == challenge_id)
                ).all()
            )
            habits = list(
                session.exec(
                    select(ChallengeTaskHabit).where(col(ChallengeTaskHabit.challenge_id) == challenge_id)
                ).all()
            )
            rewards = list(
                session.exec(
                    select(ChallengeTaskReward).where(col(ChallengeTaskReward.challenge_id) == challenge_id)
                ).all()
            )
            todos = list(
                session.exec(select(ChallengeTaskTodo).where(col(ChallengeTaskTodo.challenge_id) == challenge_id)).all()
            )
    ...
    def get_active_challenges(self, limit: int = 100) -> list[ChallengeInfo]:
        """Retrieves active challenges, optionally limited.
    ...
    def archive_completed_challenges(self) -> int:
        """Archives challenges that are marked as completed.
    ...
    def inspect_data(self) -> None:
        """Prints a comprehensive inspection report for challenge data."""
        super().inspect_data()
        with Session(self.engine) as session:
            log.info(f"\n{icons.THROPY} CHALLENGES:")
            challenges = list(session.exec(select(ChallengeInfo)).all())
            if challenges:
