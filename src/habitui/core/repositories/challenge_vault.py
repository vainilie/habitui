...
class ChallengeVault(BaseVault[ChallengeCollection]):
    ...
    def __init__(
        self,
        vault_name: str = "challenge_vault",
    ...
    def save_user_challenges_only(
        self,
        user_challenges: list[ChallengeInUser],
        strategy: SaveStrategy = "smart",
        debug: bool = False,
    ) -> None:
    ...
    def load(self) -> ChallengeCollection:
        ...
        with Session(self.engine) as session:  # type: ignore
            ...
            challenge_tasks_reward = list(
                session.exec(select(ChallengeTaskReward)).all(),
            )
    ...
    def load_challenges_by_user_id(self, user_id: str) -> list[ChallengeInfo]:
        ...
        with Session(self.engine) as session:  # type: ignore
            owned_challenges = list(
                session.exec(
                    select(ChallengeInfo).where(
                        col(ChallengeInfo.leader_id) == user_id,
                    ),
                ).all(),
            )
            user_challenge_ids = [
                uc.id for uc in session.exec(select(ChallengeInUser)).all()
            ]
            ...
            if user_challenge_ids:
                participated_challenges = list(
                    session.exec(
                        select(ChallengeInfo).where(
                            col(ChallengeInfo.id).in_(user_challenge_ids),
                        ),
                    ).all(),
                )
            all_challenges = {
                c.id: c for c in owned_challenges + participated_challenges
            }
    ...
    def get_tasks_by_challenge_id(self, challenge_id: str) -> dict[str, list]:
        ...
        with Session(self.engine) as session:  # type: ignore
            dailys = list(
                session.exec(
                    select(ChallengeTaskDaily).where(
                        col(ChallengeTaskDaily.challenge_id) == challenge_id,
                    ),
                ).all(),
            )
            habits = list(
                session.exec(
                    select(ChallengeTaskHabit).where(
                        col(ChallengeTaskHabit.challenge_id) == challenge_id,
                    ),
                ).all(),
            )
            rewards = list(
                session.exec(
                    select(ChallengeTaskReward).where(
                        col(ChallengeTaskReward.challenge_id) == challenge_id,
                    ),
                ).all(),
            )
            todos = list(
                session.exec(
                    select(ChallengeTaskTodo).where(
                        col(ChallengeTaskTodo.challenge_id) == challenge_id,
                    ),
                ).all(),
            )
            return {
    ...
    def get_legacy_challenges_id(self) -> list[str]:
        ...
        with Session(self.engine) as session:  # type: ignore
            challenges = list(
                session.exec(
                    select(ChallengeInfo).where(col(ChallengeInfo.legacy)),
                ).all(),
            )
            if challenges:
    ...
    def archive_completed_challenges(self) -> int:
        ...
        with Session(self.engine) as session:  # type: ignore
            completed_challenges = list(
                session.exec(
                    select(ChallengeInfo).where(ChallengeInfo.completed),
                ).all(),
            )
