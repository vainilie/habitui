...
class ContentVault(BaseVault[ContentCollection]):
    """Manages storage and retrieval of game content."""
    ...
    def save(self, content: ContentCollection, strategy: SaveStrategy = "smart", debug: bool = False) -> None:
        """Saves all game content from a ContentCollection to the database.
    ...
    def load(self) -> ContentCollection:
        """Loads all game content from the database into a ContentCollection."""
        with Session(self.engine) as session:
    ...
    def search_gear(self, term: str, limit: int = 10) -> list[GearItem]:
        """Searches for gear items by a given term.
    ...
    def search_quests(self, term: str, limit: int = 10) -> list[QuestItem]:
        """Searches for quest items by a given term.
    ...
    def search_spells(self, term: str, limit: int = 10) -> list[SpellItem]:
        """Searches for spell items by a given term.
    ...
    def inspect_data(self) -> None:
        """Prints a comprehensive inspection report for all game content."""
        super().inspect_data()
        with Session(self.engine) as session:
    ...
    def validate_data_integrity(self) -> list[str]:
        """Performs detailed data integrity checks for all content types.
    ...
    def _validate_gear_items(self, session: Session) -> list[str]:
        """Validates integrity of GearItem items.
    ...
    def _validate_quest_items(self, session: Session) -> list[str]:
        """Validates integrity of QuestItem items.
    ...
    def _validate_spell_items(self, session: Session) -> list[str]:
        """Validates integrity of SpellItem items.
