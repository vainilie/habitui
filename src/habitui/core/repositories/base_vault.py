...
from sqlalchemy import Engine, create_engine, delete, event, or_
...
class BaseVault[T_Collection](ABC):
    ...
    def __init__(
        ...
        log.debug("{} vault initialized with timeout {}", vault_name, cache_time)

    def _initialize_vault_metadata(self) -> None:
        """Ensure database tables are ready for the vault."""
        log.debug("Database tables ready for '{}' vault ", self.vault_name)
    ...
    def _update_vault_metadata(self, session: Session) -> None:
        ...
        log.debug("Updated vault-level metadata for '{}'", self.vault_name)

    @abstractmethod
    def save(
        self, content: T_Collection, strategy: SaveStrategy = "smart", append_mode: bool = False, debug: bool = False
    ) -> None:
    ...
    def get_paginated(
        self, model_cls: type[T_Model], page: int = 1, per_page: int = 50, order_by: str = "id"
    ) -> tuple[list[T_Model], int]:
        ...
        with Session(self.engine) as session:
            ...
            for field, value in filters.items():
                if hasattr(model_cls, field):
                    ...
                    query = (
                        query.where(column.icontains(value)) if isinstance(value, str) else query.where(column == value)
                    )
    ...
    def _get_next_available_position(self, session: Session, model_cls: type[PositionableModel]) -> int:
    ...
    def _save_single_item(
        ...
        if strategy == "force_recreate":
            ...
            session.flush()
            session.add(item)
            log.info("{} {} sync: Recreated table with 1 item.", icons.ERASE, content_name.capitalize())
        else:
            existing_item = session.get(model_cls, item.id)
            if existing_item is None:
                session.add(item)
                log.info("{} {} sync: 1 created.", icons.CREATE, content_name.capitalize())
            elif self._item_has_changed(existing_item, item, debug):
                self._update_item_fields(existing_item, item)
                log.info("{} {} sync: 1 updated.", icons.RELOAD, content_name.capitalize())
            else:
                log.info("{} {} sync: No changes.", icons.CHECK, content_name.capitalize())
    ...
    def _save_full_sync(
        ...
        """Synchronizes a list of items with the database.
        ...
        for item in items:
            ...
            normalized_item = self._normalize_datetime_fields(item)

            if existing_item is None:
                session.add(normalized_item)
                created += 1
            elif self._models_are_equivalent(existing_item, normalized_item):
                unchanged += 1
                if debug:
                    log.debug("Item {} unchanged, skipping update", item.id)
            else:
                session.merge(normalized_item)
                updated += 1
                if debug:
                    log.debug("Item {} changed, updating", item.id)
                    self._log_model_differences(existing_item, normalized_item)
        ...
        if debug or created > 0 or updated > 0 or archived_or_deleted > 0:
            log.info(
                "{} {} sync: {} created, {} updated, {} unchanged, {} {}",
                icons.CHART,
                name.capitalize(),
                created,
                updated,
                unchanged,
                archived_or_deleted,
                action if archived_or_deleted > 0 else "",
            )
        else:
            log.info("{} {} sync: No changes needed", icons.CHECK, name.capitalize())

        self._update_metadata(session, name)

    def _save_append_mode(
        self,
        session: Session,
        model_cls: type[T_Model],
        items: list[T_Model],
            ...
            if existing_item is None:
                if self._is_positionable(model_cls):
                    ...
                    if not hasattr(positionable_item, "position") or positionable_item.position is None:
                        positionable_model_cls = cast("type[PositionableModel]", model_cls)
                        positionable_item.position = self._get_next_available_position(session, positionable_model_cls)

                normalized_item = self._normalize_datetime_fields(item)
                session.add(normalized_item)
                created += 1
            elif self._models_are_equivalent(existing_item, item):
                unchanged += 1
            else:
                normalized_item = self._normalize_datetime_fields(item)
                session.merge(normalized_item)
        ...
        log.info(
            "{} {} append: {} created, {} updated, {} unchanged",
            icons.CLIP,
            name.capitalize(),
            created,
            updated,
    ...
    def _force_recreate_save(self, session: Session, model_cls: type[T_Model], items: list[T_Model], name: str) -> None:
        ...
        self._update_metadata(session, name)
        log.info("{} {} sync: Recreated table with {} items.", icons.RELOAD, name.capitalize(), len(items))

    def _archive_items(
        self,
        session: Session,
        model_cls: type[PositionableModel],
        ids_to_archive: set[Any],
        existing_items_map: dict[Any, PositionableModel],
    ) -> None:
    ...
    def _item_has_changed(self, existing: HabiTuiSQLModel, new: HabiTuiSQLModel, debug: bool = True) -> bool:
        ...
        new_dict = new.model_dump(mode="python")
        ...
        if changed and debug:
            self._print_detailed_diff(comp_existing, comp_new, existing.id)
    ...
    def _update_item_fields(existing: HabiTuiSQLModel, new: HabiTuiSQLModel) -> None:
        ...
        """
        for key, value in new.model_dump(exclude_unset=True, mode="python").items():
            setattr(existing, key, value)

    def _print_detailed_diff(self, existing_dict: dict[str, Any], new_dict: dict[str, Any], item_id: Any) -> None:
        ...
        """
        log.debug("DEBUG: Differences for item with id = '{}'", item_id)
    ...
    def _item_has_changed_after_merge(self, merged_item: T_Model, original_item: T_Model, debug: bool = False) -> bool:
        """Checks if SQLAlchemy's `merge()` operation actually resulted in changes.
        ...
        return self._item_has_changed(merged_item, original_item, debug)

    def _normalize_datetime_fields(self, item: T_Model) -> T_Model:
        """Normalizes all datetime fields of the model to UTC before merging.

        This helps prevent unnecessary updates due to timezone differences and
        ensures consistency in datetime storage and comparison.
    ...
    def _normalize_model_for_comparison(self, item: HabiTuiSQLModel) -> dict[str, Any]:
        """Normalizes a model for comparison.
    ...
    def _models_are_equivalent(self, model1: T_Model, model2: T_Model) -> bool:
        """Compares two models for equivalence.
    ...
    def _log_model_differences(self, existing: T_Model, new: T_Model) -> None:
        """Logs specific differences between two models for debugging.

        :param existing: The existing model instance.
        :param new: The new model instance.
        """
        existing_norm = self._normalize_model_for_comparison(existing)
        new_norm = self._normalize_model_for_comparison(new)
    ...
    def _configure_datetime_handling(self) -> None:
        """Configures the engine to handle datetime, especially for SQLite."""
    ...
    def search(
        self, model_cls: type[T_Model], term: str, fields: list[str] | None = None, limit: int = 10
    ) -> list[T_Model]:
        """Perform a generic search across specified text-like fields.

        :param model_cls: The SQLModel class to search.
        :param term: The search term.
        :param fields: A list of field names to search within. Defaults to
            common text fields.
        ...
        with Session(self.engine) as session:
            ...
            conditions = [
                col(getattr(model_cls, field)).icontains(term) for field in search_fields if hasattr(model_cls, field)
            ]
            if not conditions:
                return []
    ...
    def get_stats(self) -> dict[str, int]:
        ...
        :returns: A dictionary where keys are content type names suffixed with
            '_count' and values are the counts.
        """
        with Session(self.engine) as session:
            stats = {}
    ...
    def is_vault_fresh(self) -> bool:
        ...
        if vault_metadata is None or vault_metadata.last_fetched_at is None:
            log.debug("'{}' vault has no metadata - not fresh", self.vault_name)
            return False
        ...
        if total_records == 0:
            log.debug("'{}' vault has metadata but no records - not fresh", self.vault_name)
            return False
        ...
        log.debug("Vault freshness check: last_fetched={}, cutoff={}, fresh={}", last_fetched, cutoff_time, is_fresh)
    ...
    def inspect_data(self) -> None:
        ...
        log.info("Vault Name: {}", self.vault_name)

        vault_metadata = self.get_vault_metadata()
        if vault_metadata:
            vault_age = self.get_vault_age()
            log.info("Vault Last Updated: {} (age: {})", vault_metadata.last_fetched_at, vault_age or "unknown")
            log.info("Vault is Fresh: {}", icons.CHECK if self.is_vault_fresh() else icons.ERROR)

        stats = self.get_stats()
        log.info("{} RECORD COUNTS:", icons.CHART)
    ...
    def validate_data_integrity(self) -> list[str]:
        ...
        with Session(self.engine) as session:
            for name, model_cls in self.get_model_configs().items():
                stmt_invalid_ids = select(model_cls).where(or_(col(model_cls.id).is_(None), col(model_cls.id) == ""))
                ...
                stmt_duplicates = (
                    select(col(model_cls.id), func.count(col(model_cls.id)))
                    .group_by(col(model_cls.id))
                    .having(func.count(col(model_cls.id)) > 1)
                )
        ...
        if issues:
            ...
            log.debug("Data integrity check passed - no issues found")

        return issues

    def is_vault_ready_for_load(self) -> tuple[bool, list[str]]:
        """Verifies if the vault is ready for safe loading.

        Checks for data presence, freshness, and integrity.
        ...
        if total_records == 0:
            issues.append("Vault is empty - no records found")

        # 2. Check data freshness
        if not self.is_vault_fresh():
            vault_age = self.get_vault_age()
            if vault_age:
                issues.append(f"Vault data is stale - age: {vault_age} (timeout: {self.timeout})")
            else:
        ...
        if is_ready:
            log.info("{} '{}' vault  is ready for load ({} total records)", icons.CHECK, self.vault_name, total_records)
        else:
            log.warning(
                "{} '{}' vault  has {} issues preventing safe load", icons.WARNING, self.vault_name, len(issues)
            )
            for issue in issues:
                log.warning(" - {}", issue)
    ...
    def validate_datetime_fields_in_models(model_configs: dict[str, type[HabiTuiSQLModel]]) -> list[str]:
        """Validates that datetime fields in models are configured for timezone handling.

        :param model_configs: A dictionary mapping content names to their SQLModel classes.
        :returns: A list of warnings for datetime fields that might need better
        ...
        for name, model_cls in model_configs.items():
            for field_name, field_info in model_cls.model_fields.items():
                if (
                    ...
                    and "datetime" in str(field_info.annotation)
                    and not any(
                        "UTC" in str(constraint)
                        for constraint in (field_info.json_schema_extra or {}).values()
                        if isinstance(constraint, dict) and "tz" in constraint
                    )
                ):
                    warnings.append(
                        f"Model {name}.{field_name}: Consider using timezone-aware "
                        "datetime (e.g., datetime with tzinfo=UTC or Pydantic's `AwareDatetime`)"
                    )
