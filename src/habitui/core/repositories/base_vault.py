...
class BaseVault[T_Collection](ABC):
    ...
    def _update_vault_metadata(self, session: Session) -> None:
        ...
        """
        vault_metadata = ContentMetadata(
            type=self.vault_name,
    ...
    def get_paginated(
        ...
        with Session(self.engine) as session:  # type: ignore
            ...
            query = (
                select(model_cls).order_by(order_column).offset(offset).limit(per_page)
            )
        ...
        with Session(self.engine) as session:  # type: ignore
            ...
            for field, value in filters.items():
                if hasattr(model_cls, field):
                    ...
                    query = (
                        query.where(column.icontains(value))
    ...
    def _get_next_available_position(
        self,
        session: Session,
    ...
    def _get_next_archive_position(
        self,
        session: Session,
        model_cls: type[PositionableModel],
    ) -> int:
    ...
    def _save_single_item(  # noqa: PLR0917
        self,
        session: Session,
        model_cls: type[T_Model],
        item: T_Model,
        ...
        if strategy == "force_recreate":
            ...
            session.add(item)
            log.info(
                "{} {} sync: Recreated table with 1 item.",
                icons.ERASE,
    ...
    def _save_item_list(  # noqa: PLR0917
        self,
        session: Session,
        model_cls: type[T_Model],
        ...
        if strategy == "force_recreate":
            ...
            self._save_full_sync(
                session,
                model_cls,
                item_list,
                content_name,
                debug,
                use_archiving and is_positionable,
            )

        self._update_vault_metadata(session)

    def _save_full_sync(  # noqa: PLR0917
        self,
        session: Session,
        model_cls: type[T_Model],
        items: list[T_Model],
        ...
        if obsolete_ids:
            if use_archiving:
                ...
                positionable_existing_map = cast(
                    "dict[Any, PositionableModel]",
                    existing_items,
                )
                self._archive_items(
                    session,
                    positionable_model_cls,
                    obsolete_ids,
                    positionable_existing_map,
                )
                archived_or_deleted = len(obsolete_ids)
                action = "archived"
            else:
                stmt_delete = delete(model_cls).where(
                    col(model_cls.id).in_(obsolete_ids),
                )
        ...
        for item in items:
            ...
            if existing_item is None:
                if self._is_positionable(model_cls):
                    ...
                    if (
                        not hasattr(positionable_item, "position")
                        or positionable_item.position is None
                    ):
                        positionable_model_cls = cast(
                            "type[PositionableModel]",
                            model_cls,
                        )
                        positionable_item.position = self._get_next_available_position(
                            session,
                            positionable_model_cls,
                        )
    ...
    def _force_recreate_save(
        ...
        with session.no_autoflush:  # type: ignore
            log.warning(
                "Force-recreating table '{}', all data will be lost.",
        ...
        self._update_metadata(session, name)
        log.info(
            "{} {} sync: Recreated table with {} items.",
            icons.RELOAD,
            name.capitalize(),
            len(items),
        )
    ...
    def _item_has_changed(
        self,
        existing: HabiTuiSQLModel,
        new: HabiTuiSQLModel,
        debug: bool = True,
    ) -> bool:
    ...
    def _item_has_changed_after_merge(
        self,
        merged_item: T_Model,
        original_item: T_Model,
        debug: bool = False,
    ) -> bool:
        ...
        for field_name, field_value in item_dict.items():
            if isinstance(field_value, datetime):
                ...
                if field_value.tzinfo is None:
                    item_dict[field_name] = field_value.replace(
                        tzinfo=UTC,
                        microsecond=0,
                    )
                else:
                    item_dict[field_name] = field_value.astimezone(UTC).replace(
                        microsecond=0,
                    )
    ...
    def _models_are_equivalent(self, model1: T_Model, model2: T_Model) -> bool:
        ...
        return all(
            norm1.get(key) == norm2.get(key) for key in all_keys
        ...
        for key in all_keys:
            ...
            if old_val != new_val:
                if isinstance(old_val, datetime) and isinstance(new_val, datetime):
                    differences.append(
                        f"{key}: {old_val.isoformat()} -> {new_val.isoformat()}",
                    )
                else:
    ...
    def search(
        ...
        with Session(self.engine) as session:  # type: ignore
            ...
            conditions = [
                col(getattr(model_cls, field)).icontains(term)
    ...
    def is_vault_fresh(self) -> bool:
        ...
        total_records = sum(
            count for key, count in stats.items() if key.endswith("_count")
        )

        if total_records == 0:
            log.debug(
                "'{}' vault has metadata but no records - not fresh",
                self.vault_name,
            )
            return False
        ...
        last_fetched = (
            ...
            else last_fetched.astimezone(UTC)
        )
        ...
        log.debug(
            "Vault freshness check: last_fetched={}, cutoff={}, fresh={}",
            last_fetched,
            cutoff_time,
            is_fresh,
        )
    ...
    def inspect_data(self) -> None:
        ...
        if vault_metadata:
            ...
            log.info(
                "Vault Last Updated: {} (age: {})",
                vault_metadata.last_fetched_at,
                vault_age or "unknown",
            )
            log.info(
                "Vault is Fresh: {}",
                icons.CHECK if self.is_vault_fresh() else icons.ERROR,
            )
    ...
    def validate_data_integrity(self) -> list[str]:
        ...
        with Session(self.engine) as session:  # type: ignore
            for name, model_cls in self.get_model_configs().items():
                stmt_invalid_ids = select(model_cls).where(
                    or_(col(model_cls.id).is_(None), col(model_cls.id) == ""),  # noqa: PLC1901
                )
                invalid_items = list(session.exec(stmt_invalid_ids).all())

                if invalid_items:
                    issues.append(
                        f"[{name}] Found {len(invalid_items)} items with empty/null IDs",
                    )
                ...
                if self._is_positionable(model_cls):
                    ...
                    stmt_null_positions = select(positionable_model).where(
                        col(positionable_model.position).is_(None),
                    )
                    null_positions = list(session.exec(stmt_null_positions).all())

                    if null_positions:
                        issues.append(
                            f"[{name}] Found {len(null_positions)} items with null positions",
                        )
    ...
    def is_vault_ready_for_load(self) -> tuple[bool, list[str]]:
        """Verify if the vault is ready for safe loading.
        ...
        if not self.is_vault_fresh():
            ...
            if vault_age:
                issues.append(
                    f"Vault data is stale - age: {vault_age} (timeout: {self.timeout})",
                )
            else:
        ...
        if is_ready:
            log.info(
                "{} '{}' vault  is ready for load ({} total records)",
                icons.CHECK,
                self.vault_name,
                total_records,
            )
            ...
            log.warning(
                "{} '{}' vault  has {} issues preventing safe load",
                icons.WARNING,
                self.vault_name,
                len(issues),
            )
    ...
    @staticmethod
    def validate_datetime_fields_in_models(
        model_configs: dict[str, type[HabiTuiSQLModel]],
    ) -> list[str]:
