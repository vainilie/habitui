...
from habitui.core.models.task_model import TaskChecklist, TaskCollection, TaskDaily, TaskHabit, TaskReward, TaskTodo
...
class TaskVault(BaseVault[TaskCollection]):
    ...
    def save_recent_tasks(
        self, task_type: str, recent_tasks: Sequence[AnyTask], strategy: SaveStrategy = "smart", debug: bool = False
    ) -> None:
        """Saves recent tasks of a specific type without affecting older tasks.
    ...
    def save_recent_todos(self, recent_todos: list[TaskTodo], **kwargs: Any) -> None:
        """Convenience method for saving recent todos.
    ...
    def save_recent_dailys(self, recent_dailys: list[TaskDaily], **kwargs: Any) -> None:
        """Convenience method for saving recent dailys.
    ...
    def save_recent_habits(self, recent_habits: list[TaskHabit], **kwargs: Any) -> None:
        """Convenience method for saving recent habits.
    ...
    def save_recent_rewards(self, recent_rewards: list[TaskReward], **kwargs: Any) -> None:
        """Convenience method for saving recent rewards.
    ...
    def save_recent_subtasks(self, recent_subtasks: list[TaskChecklist], **kwargs: Any) -> None:
        """Convenience method for saving recent subtasks.
    ...
    def load(self) -> TaskCollection:
        """Loads all active task content from the database.
        ...
        with Session(self.engine) as session:
            ...
            return TaskCollection(
                todos=todos, dailys=dailys, habits=habits, rewards=rewards, subtasks=subtasks, challenges=[]
            )

    def get_active_tasks(self, task_type: str, limit: int = 100) -> Sequence[AnyTask]:
        """Retrieves active (non-archived) tasks of a specific type.
        ...
        with Session(self.engine) as session:
            ...
            query = (
                select(model_class)
                .where(model_class.position < self.ARCHIVE_POSITION_START)
    ...
    def get_active_todos(self, limit: int = 100) -> Sequence[AnyTask]:
        """Convenience method for getting active todos.
    ...
    def get_active_dailys(self, limit: int = 100) -> Sequence[AnyTask]:
        """Convenience method for getting active dailys.
    ...
    def get_active_rewards(self, limit: int = 100) -> Sequence[AnyTask]:
        """Convenience method for getting active rewards.
    ...
    def get_active_subtasks(self, limit: int = 100) -> Sequence[AnyTask]:
        """Convenience method for getting active subtasks.
    ...
    def archive_tasks_by_count(self, task_type: str, keep_count: int = 1000) -> int:
        """Archives older tasks of a specific type. Keeps only the most recent ones.
    ...
    def archive_all_old_tasks(self, keep_count: int = 500) -> dict[str, int]:
        """Archives old tasks for all task types.
    ...
    def inspect_data(self) -> None:
        """Prints a comprehensive inspection report for task data."""
        super().inspect_data()
        with Session(self.engine) as session:
            log.info(f"\n{icons.INFO} TASK SUMMARY:")
            for task_type, model_class in self.get_model_configs().items():
                if hasattr(model_class, "position"):
                    active_count = len(
                        session.exec(
                            select(model_class).where(model_class.position < self.ARCHIVE_POSITION_START)
                        ).all()
                    )
                    archived_count = len(
                        session.exec(
            ...
            log.info(f"\n{icons.INFO} RECENT TODOS (first 3):")
            ...
            log.info(f"\n{icons.INFO} RECENT dailys (first 3):")
