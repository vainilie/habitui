...
class TaskVault(BaseVault[TaskCollection]):
    ...
    def save_recent_rewards(
        self,
        recent_rewards: list[TaskReward],
        **kwargs: Any,
    ) -> None:
        """Save recent rewards.
    ...
    def save_recent_subtasks(
        self,
        recent_subtasks: list[TaskChecklist],
        **kwargs: Any,
    ) -> None:
    ...
    def load(self) -> TaskCollection:
        ...
        with Session(self.engine) as session:  # type: ignore
            ...
            def load_active(model: type[T]) -> list[T]:
                ...
                stmt = (
                    select(model)
                    .where(position_col < self.ARCHIVE_POSITION_START)
                    ...
                    active_count = len(
                        session.exec(
                            select(model_class).where(
                                model_class.position < self.ARCHIVE_POSITION_START,
                            ),
                        ).all(),
                    )
                    archived_count = len(
