...
class ChallengeTaskBase(HabiTuiSQLModel, table=False):
    ...
    challenge: dict[str, Any] = Field(default_factory=dict, sa_column=Column(validators.PydanticJSON))
    group: dict[str, Any] = Field(default_factory=dict, sa_column=Column(validators.PydanticJSON))
    reminders: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
...
class ChallengeInfo(HabiTuiSQLModel, table=True):
    ...
    categories: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    habits: list[str] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    dailys: list[str] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    todos: list[str] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    rewards: list[str] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))

    @field_validator("name", "short_name", "summary", "description", "leader_name", "group_name", mode="before")
    @classmethod
    def _clean_text(cls, v: Any) -> str:
        """Clean text fields by replacing emoji shortcodes."""
        return validators.replace_emoji_shortcodes(v)
    ...
    @classmethod
    def flatten_api_data(cls, data: Box, user_id: str | None = None, user_challenge_ids: set[str] | None = None, task_challenge_ids: set[str] | None = None) -> dict[str, Any]:
        ...
        if user_id and user_challenge_ids is not None and task_challenge_ids is not None:
            flat_data["owned"] = data.leader and data.leader.id == user_id
            flat_data["joined"] = data.id in user_challenge_ids or data.id in task_challenge_ids
            flat_data["left"] = data.id in user_challenge_ids and data.id not in task_challenge_ids
        return flat_data.to_dict()

    @classmethod
    def from_api_data(cls, data: Box, user_context: UserCollection | None, task_challenge_ids: set[str]) -> Self:
        ...
        user_challenge_ids = {c.id for c in user_context.challenges} if user_context else set()
        flat_data = cls.flatten_api_data(data, user_id, user_challenge_ids, task_challenge_ids)
...
class ChallengeTaskTodo(ChallengeTaskBase, table=True):
    ...
    checklist: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    challenge: dict[str, Any] = Field(default_factory=dict, sa_column=Column(validators.PydanticJSON))
    group: dict[str, Any] = Field(default_factory=dict, sa_column=Column(validators.PydanticJSON))
    reminders: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
...
class ChallengeTaskDaily(ChallengeTaskBase, table=True):
    ...
    checklist: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    repeat: dict[str, bool] = Field(default_factory=dict, sa_column=Column(validators.PydanticJSON))
    days_of_month: list[int] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    weeks_of_month: list[int] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    next_due: list[datetime.datetime | None] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    challenge: dict[str, Any] = Field(default_factory=dict, sa_column=Column(validators.PydanticJSON))
    group: dict[str, Any] = Field(default_factory=dict, sa_column=Column(validators.PydanticJSON))
    reminders: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(validators.PydanticJSON))
    ...
    def _parse_next_due_dates(cls, value: Any) -> list[datetime.datetime | None]:
        ...
        if isinstance(value, list):
            return [validators.parse_datetime(date_str) for date_str in value if date_str is not None]
        return []
...
class ChallengeCollection(HabiTuiBaseModel):
    ...
    @classmethod
    def from_api_data(cls, challenges_data: list[dict[str, Any]] | None = None, challenge_tasks_data: list[dict[str, Any]] | None = None, user: UserCollection | None = None, tasks: TaskCollection | None = None) -> Self:
        ...
        if challenges_data and challenge_tasks_data:
            return cls.from_combined_data(challenges_data, challenge_tasks_data, user, tasks)
        if challenges_data:
        ...
        return cls(user_challenges=user.challenges if user else [], task_challenges=tasks.challenges if tasks else [])

    @classmethod
    def from_challenges_data(cls, raw_challenges: list[dict], user: UserCollection | None, tasks: TaskCollection | None) -> Self:
        ...
        task_challenge_ids = {c.challenge_id for c in tasks.all_tasks if c.challenge_id} if tasks else set()
        parsed_challenges = cls._parse_challenges_list(raw_challenges, user, user_challenge_ids, task_challenge_ids)
        return cls(challenges=parsed_challenges, user_challenges=user.challenges if user else [], task_challenges=tasks.challenges if tasks else [])

    @classmethod
    def from_challenge_tasks_data(cls, raw_tasks: list[dict], user: UserCollection | None = None, tasks: TaskCollection | None = None) -> Self:
        ...
        return cls(user_challenges=user.challenges if user else [], task_challenges=tasks.challenges if tasks else [], challenge_tasks_daily=dailies, challenge_tasks_habit=habits, challenge_tasks_reward=rewards, challenge_tasks_todo=todos)

    @classmethod
    def from_combined_data(cls, raw_challenges: list[dict[str, Any]], raw_tasks: list[dict[str, Any]], user: UserCollection | None, tasks: TaskCollection | None) -> Self:
        ...
        :returns: A populated `ChallengeCollection` instance.
        """
        collection_from_challenges = cls.from_challenges_data(raw_challenges, user, tasks)
        dailies, habits, rewards, todos = cls._parse_tasks_list(raw_tasks)
    ...
    @staticmethod
    def _parse_challenges_list(raw_challenges: list[dict], user: UserCollection | None, user_challenge_ids: set[str], task_challenge_ids: set[str]) -> list[ChallengeInfo]:
        ...
        for raw_challenge in raw_challenges:
            try:
                ...
                flat_data = ChallengeInfo.flatten_api_data(data, user_id, user_challenge_ids, task_challenge_ids)
                parsed_list.append(ChallengeInfo.model_validate(flat_data))
            except (ValidationError, KeyError) as e:
                log.error("Failed to parse challenge {}: {}", raw_challenge.get("id", "N/A"), e)
        return parsed_list

    @staticmethod
    def _parse_tasks_list(raw_tasks: list[dict]) -> tuple[list[ChallengeTaskDaily], list[ChallengeTaskHabit], list[ChallengeTaskReward], list[ChallengeTaskTodo]]:
        ...
        """
        collections: dict[str, list[Any]] = {"daily": [], "habit": [], "reward": [], "todo": []}
        factories = {"daily": ChallengeTaskDaily.model_validate, "habit": ChallengeTaskHabit.model_validate, "reward": ChallengeTaskReward.model_validate, "todo": ChallengeTaskTodo.model_validate}
        for raw_task in raw_tasks:
            try:
                ...
                log.error("Failed to parse challenge task {}: {}", raw_task.get("id", "N/A"), e)
        return collections["daily"], collections["habit"], collections["reward"], collections["todo"]
    ...
    def add_challenge_from_dict(self, challenge_data: dict[str, Any], user: UserCollection | None = None, tasks: TaskCollection | None = None) -> bool:
    ...
    def get_joined_challenges(self) -> dict[str, ChallengeInfo]:
        ...
        return {c.id: c for c in sorted((c for c in self.challenges if c.joined), key=lambda c: c.created_at or datetime.datetime.min, reverse=True)}

    def get_owned_challenges(self) -> dict[str, ChallengeInfo]:
        """Get a dictionary of all challenges the user owns."""
        return {c.id: c for c in sorted((c for c in self.challenges if c.owned), key=lambda c: c.created_at or datetime.datetime.min, reverse=True)}

    def get_all_challenges(self) -> dict[str, ChallengeInfo]:
        """Get a dictionary of all challenges."""
        return {c.id: c for c in sorted(self.challenges, key=lambda c: c.created_at or datetime.datetime.min, reverse=True)}

    def get_legacy_challenges(self) -> dict[str, ChallengeInfo]:
        """Get a dictionary of all legacy challenges."""
        return {c.id: c for c in sorted((c for c in self.challenges if c.legacy), key=lambda c: c.created_at or datetime.datetime.min, reverse=True)}

    @property
    def all_tasks(self) -> list[Any]:
        """Return a single list containing all challenge tasks."""
        return [*self.challenge_tasks_todo, *self.challenge_tasks_daily, *self.challenge_tasks_habit, *self.challenge_tasks_reward]
