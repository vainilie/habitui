...
from .task_model import ChallengeInTask, TaskCollection
from .user_model import ChallengeInUser, UserCollection
...
class ChallengeTaskBase(HabiTuiSQLModel, table=False):
    ...
    def _clean_text(cls, v: Any) -> str:
        """Cleans text fields by replacing emoji shortcodes.
...
class ChallengeInfo(HabiTuiSQLModel, table=True):
    """Represents a single Habitica Challenge and its metadata.
    ...
    def _flatten_api_data(
        ...
        """Flattens Box API data into a dictionary suitable for model validation.
    ...
    def from_api_data(cls, data: Box, user_context: UserCollection | None, task_challenge_ids: set[str]) -> Self:
        """Creates a ChallengeInfo instance from API data, including relationship status.
        ...
        flat_data = cls._flatten_api_data(data, user_id, user_challenge_ids, task_challenge_ids)
...
class ChallengeTaskTodo(ChallengeTaskBase, table=True):
    ...
    def _parse_due_date(cls, v: Any) -> datetime.datetime | None:
        """Parses the due date string into a datetime object.
...
class ChallengeTaskDaily(ChallengeTaskBase, table=True):
    ...
    def _parse_start_date(cls, v: Any) -> datetime.datetime | None:
        """Parses the start date string into a datetime object.
    ...
    def _parse_next_due_dates(cls, value: Any) -> list[datetime.datetime | None]:
        """Parses a list of next due date strings into datetime objects.
...
class ChallengeCollection(HabiTuiBaseModel):
    ...
    def from_api_data(
        ...
        """Parses raw API data into a ChallengeCollection.
    ...
    def from_challenges_data(
        cls, raw_challenges: list[dict], user: UserCollection | None, tasks: TaskCollection | None
    ) -> Self:
        """Creates a collection from challenge metadata only.
    ...
    def from_challenge_tasks_data(
        cls, raw_tasks: list[dict], user: UserCollection | None = None, tasks: TaskCollection | None = None
    ) -> Self:
        """Creates a collection from challenge task data only.
    ...
    def from_combined_data(
        ...
        """Creates a collection from both challenge metadata and task data.
    ...
    def _parse_challenges_list(
        ...
        """Parses a list of raw challenge dictionaries into ChallengeInfo models.
        ...
        for raw_challenge in raw_challenges:
            try:
                ...
                flat_data = ChallengeInfo._flatten_api_data(  # noqa: SLF001
                    data, user_id, user_challenge_ids, task_challenge_ids
                )
    ...
    def _parse_tasks_list(
        raw_tasks: list[dict],
    ) -> tuple[list[ChallengeTaskDaily], list[ChallengeTaskHabit], list[ChallengeTaskReward], list[ChallengeTaskTodo]]:
        """Parses and categorizes a list of raw challenge task dictionaries.

        :param raw_tasks: List of raw challenge task dictionaries.
        :returns: A tuple of lists, each containing `ChallengeTaskDaily`, `ChallengeTaskHabit`, `ChallengeTaskReward`, and `ChallengeTaskTodo`.
        """
    ...
    def add_challenge_from_dict(
        self, challenge_data: dict[str, Any], user: UserCollection | None = None, tasks: TaskCollection | None = None
    ) -> bool:
        """Adds a challenge from a dictionary.
    ...
    def find_challenge_by_id(self, challenge_id: str) -> ChallengeInfo | None:
        """Finds a challenge by its ID using next() for efficiency.
    ...
    def get_challenge_by_id_with_validation(
        self, challenge_id: str, raise_if_not_found: bool = False
    ) -> ChallengeInfo | None:
        """Gets a challenge by its ID with additional validation.
    ...
    def remove_challenge_by_id_efficient(self, challenge_id: str) -> bool:
        """Removes a challenge by its ID efficiently.
