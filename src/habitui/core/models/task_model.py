...
from .validators import PydanticJSON, calculate_task_damage, habit_status, normalize_attribute, replace_emoji_shortcodes
...
class ChallengeInTask(HabiTuiSQLModel, table=True):
    """Represents a challenge linked to a task."""
...
class TaskChecklist(HabiTuiSQLModel, table=True):
    ...
    def validate_text_fields(cls, v: Any) -> str:
        """Validates and cleans text fields by replacing emoji shortcodes.
...
class TaskReward(HabiTuiSQLModel, table=True):
    ...
    def validate_attribute(cls, v: Any) -> str | None:
        """Validates and normalizes attribute names.
...
class TaskDaily(HabiTuiSQLModel, table=True):
    ...
    def validate_datetime(cls, v: Any) -> list[str]:
        """Validates and formats datetime objects in a list.
    ...
    def from_api_dict(cls, data: Box, user: UserCollection) -> TaskDaily:
        """Creates a TaskDaily instance from API data.
...
class TaskCollection(HabiTuiBaseModel):
    ...
    def all_tasks(self) -> list[AnyTask]:
        """Returns a single list containing all primary tasks."""
    ...
    def from_api_data(cls, raw_content: SuccessfulResponseData, user_vault: UserCollection) -> TaskCollection:
        """Parses the raw API response and creates a TaskCollection instance.
    ...
    def add_task(self, task: AnyTask) -> None:
        """Adds a new task to the appropriate collection list based on its type.
    ...
    def score_task(self, task_id: str, direction: str = "up") -> AnyTask | None:
        """Simulates scoring a task, updating its local state.
    ...
    def get_task_by_id(self, task_id: str) -> AnyTask | None:
        """Finds a task by its ID across all task types.
    ...
    def get_tasks_by_status(self, status: TaskStatusType) -> list[AnyTask]:
        """Gets all tasks that match a specific status enum.
    ...
    def get_tasks_by_challenge_id(self, challenge_id: str) -> list[AnyTask]:
        """Gets all tasks associated with a given challenge ID.
    ...
    def get_tasks_by_broken_status(
        self, is_task_broken: bool | None = None, is_challenge_broken: bool | None = None
    ) -> list[AnyTask]:
        """Gets tasks by their broken status.
    ...
    def get_tasks_by_date(self, due_date: date) -> list[TaskTodo | TaskDaily]:
        """Gets all Todos and Dailies due on a specific date.
        ...
        due_dailies = [
            ...
            if any(
                DateTimeHandler(timestamp=d).local_datetime and DateTimeHandler(timestamp=d).local_datetime == due_date
    ...
    def get_due_dailies(self) -> list[TaskDaily]:
        """Gets all daily tasks that are currently due and not completed.
    ...
    def add_checklist_item(
        self, task_id: str, text: str, completed: bool = False
    ) -> tuple[TaskChecklist | None, AnyTask | None] | None:
        """Adds a new subtask (checklist item) to a Todo or Daily.
        ...
        :returns: A tuple containing the newly created TaskChecklist instance
                  and the updated parent task, or None if the parent task is invalid.
        """
    ...
    def __len__(self) -> int:
        """Returns the total number of primary tasks."""
        return len(self.all_tasks)

    def __iter__(self) -> Iterator[AnyTask]:
        """Returns an iterator over all primary tasks."""
        return iter(self.all_tasks)

    def __getitem__(self, index: int | slice) -> AnyTask | list[AnyTask]:
        """Retrieves a task or slice of tasks from the combined list."""
