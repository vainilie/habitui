...
class TaskProcessor:
    ...
    def process_tasks(self, raw_content: list[dict[str, Any]]) -> TaskCollection:
        ...
        for ch in self.challenge_map.values():
            self.collections["challenges_tasks_ids"].append(
                ChallengeInTask.model_validate(ch),
            )
    ...
    def _process_challenge(self, task_data: Box) -> None:
        ...
        """
        challenge_model, challenge_in_task = parse_nested_challenge(
            task_data.challenge,
            task_data.id,
        )
        ...
        if challenge_model:
            ...
            if challenge_id not in self.challenge_map:
                ...
                existing["tasks_ids"] = sorted(
                    set(existing.get("tasks_ids", []))
                    | set(challenge_model.get("tasks_ids", [])),
                )
                existing["broken_tasks_ids"] = sorted(
                    set(existing.get("broken_tasks_ids", []))
                    | set(challenge_model.get("broken_tasks_ids", [])),
                )
    ...
    def _parse_reward(self, task_data: Box) -> TaskReward:
        ...
        if (
            self.user_vault
            and task_data.get("value", 0) <= self.user_vault.raw_stats.gp
        ):
    ...
    def _parse_todo(self, task_data: Box) -> TaskTodo:
        ...
        task_data["status"] = todo_status(
            task_data.completed,
            task_data.checklist_progress,
            task_data.due_date,
        )
