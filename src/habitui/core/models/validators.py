...
from .base_enums import DailyStatus, HabitStatus, TaskStatus, TodoStatus
...
def replace_emoji_shortcodes(value: Any) -> str:
    """Replaces emoji shortcodes (e.g., :smile:) with Unicode characters."""
    return emoji_data_python.replace_colons(str(value or "")).strip()


def normalize_attribute(value: str | None) -> str | None:
    """Converts abbreviated character attributes to their full names."""
    if not value:
        return None
...
def parse_datetime(value: Any) -> datetime.datetime | None:
    """Parses a string or other type into a timezone-aware UTC datetime object."""
    if not value:
        return None
...
def parse_to_float(value: Any, default: float = 0.0) -> float:
    """Converts a value to a float, returning a default on failure."""
    try:
        return float(value)
...
def _determine_habit_status_by_value(value: float) -> HabitStatus:
    """Determines habit status based on a numerical value."""
...
def _determine_negative_habit_status(value: float) -> HabitStatus:
    """Determines status for a negative habit based on its value."""
    if value < LOWEST_VALUE:
        return HabitStatus.STRUGGLING
    if value < LOW_VALUE:
        return HabitStatus.EFFORT
...
def _determine_positive_habit_status(value: float) -> HabitStatus:
    """Determines status for a positive habit based on its value."""
    if value > HIGHEST_VALUE:
        return HabitStatus.FLOW
    if value > HIGH_VALUE:
        return HabitStatus.ON_TRACK
...
def habit_status(up: bool, down: bool, value: float) -> HabitStatus:
    """Calculates the overall status for a habit."""
...
def _get_date_based_todo_status(due_date: datetime.datetime) -> TodoStatus:
    """Determines ToDo status based on its due date."""
...
def todo_status(completed: bool, checklist_progress: float, due_date: datetime.datetime | None) -> TodoStatus:
    """Calculates the overall status for a todo."""
    if completed:
...
def _get_daily_completion_status(completed: bool, due_today: bool, due_yesterday: bool) -> DailyStatus | None:
    """Helper to determine daily completion status."""
    if completed:
        if due_today:
...
def _get_daily_progress_status(checklist_progress: float) -> DailyStatus:
    """Helper to determine daily progress status."""
    if checklist_progress >= 1.0:
...
def daily_status(completed: bool, due_today: bool, due_yesterday: bool, checklist_progress: float) -> DailyStatus:
    """Calculates the overall status for a daily."""
...
def calculate_task_damage(data: Box, user: UserCollection) -> None:
    """Calculates and sets user and party damage on the task data."""
...
def extract_subtasks(
    raw_checklist: list[Box] | None, parent_task_id: str
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """Extracts subtasks, calculates stats, and prepares data for model creation."""
    ...
    for i, item in enumerate(raw_checklist):
        ...
        subtask_models_data.append({
            "id": item.id,
            "text": item.text,
            "completed": is_completed,
            "status": TaskStatus.COMPLETED if is_completed else TaskStatus.AVAILABLE,
    ...
    stats.update({
        ...
        "checklist": [item["id"] for item in subtask_models_data],
    })
...
def parse_nested_challenge(challenge_box: Box | None, task_id: str) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    """Parses nested challenge data from API response."""
    task_updates = {"challenge": False}
    ...
    new_updates = {
        "challenge": True,
        "challenge_id": challenge_box.id,
        "challenge_task_id": challenge_box.task_id,
        "challenge_shortname": challenge_box.short_name,
        "challenge_broken": False,
        "task_broken": False,
    ...
    challenge_model = {
        "id": challenge_box.id,
        "short_name": challenge_box.short_name,
        "broken": task_updates["challenge_broken"],
        "winner": task_updates["challenge_winner"],
        "tasks_ids": [task_id],
        "broken_tasks_ids": broken_tasks,
    }
    return challenge_model, task_updates


def clean_tag_name(name: str) -> str:
    """Cleans a tag name by replacing emoji shortcodes and stripping whitespace."""
