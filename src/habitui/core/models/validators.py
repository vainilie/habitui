...
def _get_daily_completion_status(
    completed: bool,
    due_today: bool,
    due_yesterday: bool,
) -> DailyStatus | None:
...
def calculate_task_damage(data: Box, user: UserCollection) -> None:
    ...
    hp_damage_to_user = (
        effective_damage_factor * user_con_mitigation * data.priority * 2.0
    )
    ...
    if boss_strength and boss_strength > 0:
        ...
        party_damage_factor = (
            effective_damage_factor * data.priority
            if data.priority < 1
            else effective_damage_factor
        )
...
def parse_nested_challenge(
    challenge_box: Box | None,
    task_id: str,
