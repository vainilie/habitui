...
from .content_model import ContentCollection, QuestItem, SpellItem
...
class ChallengeInUser(HabiTuiSQLModel, table=True):
    """Represents a challenge a user is participating in.
    ...
    def from_api_id(cls, challenge_id: str) -> Self:
        """Creates an instance from a challenge ID string.
...
class UserProfile(HabiTuiSQLModel, table=True):
    ...
    def from_api_box(cls, data: Box) -> Self:
        """Creates an instance from the main API data box.
...
class UserHistory(HabiTuiSQLModel, table=True):
    ...
    @classmethod
    def from_api_box(cls, data: Box) -> Self:
        """Creates an instance from the history sub-object.
...
class UserTasksOrder(HabiTuiSQLModel, table=True):
    ...
    @classmethod
    def from_api_box(cls, data: Box) -> Self:
        """Creates an instance from the tasksOrder sub-object.
...
class UserPreferences(HabiTuiSQLModel, table=True):
    ...
    @classmethod
    def from_api_box(cls, data: Box) -> Self:
        """Creates an instance from the preferences sub-object.
...
class UserAchievements(HabiTuiSQLModel, table=True):
    ...
    @classmethod
    def from_api_box(cls, data: Box) -> Self:
        """Creates an instance from the achievements sub-object.
...
class UserNotifications(HabiTuiSQLModel, table=True):
    ...
    @classmethod
    def from_api_box(cls, data: Box) -> Self:
        """Creates an instance from various notification-related fields.
...
class UserTimestamps(HabiTuiSQLModel, table=True):
    ...
    @classmethod
    def from_api_box(cls, data: Box) -> Self:
        """Creates an instance from the auth.timestamps sub-object.
...
class UserStatsComputed(HabiTuiSQLModel, table=True):
    ...
    def from_dependencies(
        cls, user_id: str, raw_stats: UserStatsRaw, user_state: UserCurrentState, content_vault: ContentCollection
    ) -> Self:
        """Calculates all effective stats and returns an instance.
...
class UserCollection(HabiTuiBaseModel):
    ...
    def from_api_data(cls, raw_data: dict[str, Any], content_vault: ContentCollection) -> Self:
        """Parses raw API data into a structured UserCollection instance.
        ...
        computed_stats = UserStatsComputed.from_dependencies(
            user_id=data.id, raw_stats=raw_stats, user_state=user_state, content_vault=content_vault
        )
    ...
    def get_current_quest_data(self, content_vault: ContentCollection) -> QuestItem | None:
        ...
        if self.user_state.current_quest_key:
            return content_vault.get_quest(key=self.user_state.current_quest_key)  # o como accedas
        return None

    def available_spells(self, content_vault: ContentCollection) -> dict[str, SpellItem | int]:
        """Get current quest data from game content."""
