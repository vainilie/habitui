...
"""Define Pydantic models used for creating new entities via the Habitica API."""
...
from .base_enums import Priority, TaskType, Attribute, Frequency
...
class ChecklistItemCreate(HabiTuiBaseModel):
	"""Represent an individual checklist item during task creation.
	...
	def _parse_text(cls, value: Any) -> str:
		"""Clean and replaces emoji shortcodes in the text.
...
class ReminderCreate(HabiTuiBaseModel):
	"""Represent a reminder to be set during task creation.
...
class DailyRepeatPattern(HabiTuiBaseModel):
	"""Define the weekly repeat pattern for a Daily task.
