...
class TaskFilter:
    ...
    def get_challenge_filter_options(
        self,
...
class TasksTab(Vertical, BaseTab):
    ...
    def get_filter_settings(self) -> dict[str, str]:
        ...
        return {
            ...
            "challenge_filter": self.current_challenge_filter,
    ...
    def compose(self) -> ComposeResult:
        ...
        current_tasks = self.filter.get_tasks_for_mode(
            self.tasks,
            self.current_mode,
    ...
    async def _score_selected_tasks(self, direction: str = "auto") -> None:
        ...
        if result["error"] == 0:
            self.notify(
                f"{icons.CHECK} {result['success']} tasks scored!",
                title="Tasks Scored",
            )
        else:
            self.notify(
                f"{icons.WARNING} {result['success']} scored, {result['error']} failed.",
    ...
    @work
    async def _delete_tasks_with_confirmation(
        self,
        tasks_to_delete: list[AnyTask],
    ) -> None:
        ...
        if result["error"] == 0:
            self.notify(
                f"{icons.CHECK} {result['success']} tasks deleted!",
                title="Tasks Deleted",
            )
        else:
            self.notify(
                f"{icons.WARNING} {result['success']} deleted, {result['error']} failed.",
                title="Batch Complete",
            )
            ...
            if task := self.get_task(task_id):
                if action_func == self.operations.score_tasks:
                    ...
                    if result["error"] == 0:
                        ...
                        self.notify(
                            f"{icons.ERROR} Failed to score task",
                            title="Error",
                            severity="error",
                        )
                elif action_func == self._delete_tasks_with_confirmation:
