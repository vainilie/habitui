...
class TagsTab(BaseTab):
    ...
    tags_collection: reactive[TagCollection] = reactive(None, recompose=True)  # type: ignore
    ...
    def _create_tags_hierarchy_panel(self) -> Panel:
        ...
        return Panel(tree, title="Tag Hierarchy", title_icon="TREE", element_id="tags-hierarchy-panel")
    ...
    def _populate_tree_nodes(self, ownership_node, attribute_node, free_tags_node) -> None:
        ...
        def sort_tags(tags: list[TagComplex]) -> list[TagComplex]:
    ...
    async def _tag_tasks_workflow(self) -> None:
        ...
        del_ch_tag = []
        del_pe_tag = []
        for task in self.vault.ensure_tasks_loaded().get_challenge_tasks():
            if task.challenge and challenge_tag and challenge_tag.id not in task.tags:
                add_ch_tag.append(task.id)
            if task.challenge and personal_tag and personal_tag.id in task.tags:
                del_pe_tag.append(task.id)
        for task in self.vault.ensure_tasks_loaded().get_owned_tasks():
            if not task.challenge and challenge_tag and challenge_tag.id in task.tags:
                del_ch_tag.append(task.id)
            if not task.challenge and personal_tag and personal_tag.id not in task.tags:
    ...
    async def _tag_tasks_via_api(self, changes: dict) -> None:
        try:
            ...
            for task_id in changes["add"]["challenge"]:
                await self.app.habitica_api.add_tag_to_task(task_id=task_id, tag_id_to_add=changes["ch_tag"])
            for task_id in changes["add"]["personal"]:
                await self.app.habitica_api.add_tag_to_task(task_id=task_id, tag_id_to_add=changes["pe_tag"])
            for task_id in changes["del"]["challenge"]:
                await self.app.habitica_api.remove_tag_from_task(task_id=task_id, tag_id_to_remove=changes["ch_tag"])
            for task_id in changes["del"]["personal"]:
                await self.app.habitica_api.remove_tag_from_task(task_id=task_id, tag_id_to_remove=changes["pe_tag"])
        except Exception as e:
            log.error(f"{icons.ERROR} Error adding/deleting tag: {e}")
            self.notify(f"{icons.ERROR} Failed to add/del tag: {e!s}", title="Error", severity="error")
    ...
    async def _create_tag_via_api(self, changes: dict) -> None:
        ...
        try:
            ...
            if payload:
                ...
                self.refresh_data()
                self.notify(f"{icons.CHECK} Tag created successfully!", title="Tag Created", severity="information")
                log.info(f"{icons.CHECK} Tag created.")
            else:
                self.notify(f"{icons.INFO} No tag data to create.", title="Tag Creation", severity="information")
        except Exception as e:
            log.error(f"{icons.ERROR} Error creating tag: {e}")
            self.notify(f"{icons.ERROR} Failed to create tag: {e!s}", title="Error", severity="error")
    ...
    async def _update_tag_via_api(self, changes: dict, tag_id: str) -> None:
        ...
        try:
            ...
            if payload:
                ...
                self.notify(f"{icons.INFO} No changes to save.", title="Tag Update", severity="information")
        except Exception as e:
            log.error(f"{icons.ERROR} Error updating tag: {e}")
            self.notify(f"{icons.ERROR} Failed to update tag: {e!s}", title="Error", severity="error")
    ...
    async def _delete_tag_via_api(self, tag: TagComplex) -> None:
        ...
        try:
            ...
            self.notify(f"{icons.ERROR} Failed to delete tag: {e!s}", title="Error", severity="error")
