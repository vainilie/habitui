...
class BaseTab(Widget):
	...
	def on_tab_activated(self) -> None:
		"""Called when tab becomes active - refreshes data automatically."""
	...
	def _create_icon_value_row(self, row_id: str, icon: str, text_id: str) -> ComposeResult:
	...
	def _create_progress_bar_row(self, stat_type: str, label: str, icon: str, css_class: str) -> ComposeResult:
		...
		with Horizontal(classes="icon-label-bar-row", id=f"{stat_type}-stats-row"):
			...
			yield Label("0/0", id=f"{stat_type}-progress-text", classes=f"value {css_class}")

	def _create_icon_label_value_row(self, row_id: str, icon: str, label: str, text_id: str) -> ComposeResult:
	...
	def _create_label_value_row(self, row_id: str, label: str, text_id: str) -> ComposeResult:
		"""Create a label-value row."""
		with Horizontal(classes="label-value-row", id=row_id):
			yield Label(label, classes="label")
			yield Label("Loading...", classes="value", id=text_id)

	def _update_progress_bar_widget(self, bar_id: str, text_id: str, current: int, total: int) -> None:
		"""Helper to update a single progress bar and its associated text."""
		try:
