                ...
                for field in self.fields:
                    ...
                    if field.field_type == FieldType.TEXT:
                        ...
                        yield TextArea.code_editor(
                            ...
                            id=field.id,
                            classes=f"input-box {field.classes}",
                        )
                        ...
                        select_widget = Select(
                            ...
                            id=field.id,
                            classes=f"input-select {field.classes}",
                        )
                        yield select_widget
                        ...
                        yield Switch(
                            ...
                            id=field.id,
                            classes=f"input-switch {field.classes}",
                        )

                    elif field.field_type == FieldType.STATIC:
                        yield Static(
                            field.label,
                            classes=f"input-static {field.classes}",
                        )
        ...
        if field.field_type == FieldType.INTEGER:
            try:
                ...
                if field.max_value is not None and int_value > field.max_value:
                    return (
                        False,
                        f"{field.label} must be no more than {field.max_value}",
                    )
            except ValueError:
...
def create_settings_modal() -> GenericEditModal:
    ...
    fields = [
        ...
        FormField(
            ...
            label="API URL:",
            field_type=FieldType.TEXT,
            placeholder="https://habitica.com/api/v3",
        ),
        FormField(
            id="info",
            label="Settings are saved automatically",
            field_type=FieldType.STATIC,
