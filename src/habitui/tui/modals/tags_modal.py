...
def create_batch_tag_delete_modal(
    tag_list: list[TagComplex],
    tag_uses: dict,
) -> GenericConfirmModal:
    """Create confirmation modal for batch tag deletion."""
    tag_list_render = [
        f"• {tag.name} ({tag_uses.get(tag.id, 0)} tasks affected)\n" for tag in tag_list
    ]
    return GenericConfirmModal(
        question=f"Are you sure you want to delete {len(tag_list)} selected tags?",
        title="Batch Delete Tags",
        confirm_text=f"Delete {len(tag_list)} Tags",
        cancel_text="Cancel",
        confirm_variant="error",
        icon=icons.WARNING,
        custom_content=tag_list_render,
    )
...
def create_reorder_confirm_modal() -> GenericConfirmModal:
    ...
    return GenericConfirmModal(
        question="Apply new order to all tags?",
